...
{
    public partial class BFRun2_Rep_fal : System.Web.UI.Page
    {
        ...
        protected void GetData(string strHeight)
        {
            ...
            strSQL += "max(accum_charge) as accum_charge,";
            ...
            strSQL += "round(avg(BLAST_AMOUNT-350),2) as BLAST_AMOUNT  ,";
            ...
            strSQL += "round(avg((BLAST_AMOUNT-350) / P_ALL),2) as TQX   ,";
            strSQL += "round(avg(GAS_USE),2) as GAS_USE ,";
            strSQL += "round(avg(GAS_AMOUNT),2) as GAS_AMOUNT  ,";
            strSQL += "round(avg(RTRIM(TO_CHAR((0.78 * NVL(RICH_OXY_AMOUNT, 0) / (BLAST_AMOUNT-350) / 60 * 100), 'FM9990.9'), '.')),2) as RICH_OXY_RATIO  ,";
            ...
            strSQL += "round(avg(RTRIM(TO_CHAR(COKE_JET1*1000/158.3,'FM9990.9'),'.')),2) as COKE_RATIO  ,";
            strSQL += "round(avg(SPEED_LMXJ),2) as SPEED_LMXJ  ,";
            strSQL += "round(sum(RTRIM(TO_CHAR(COKE_JET1,'FM9990.9'),'.')),2) as COKE_JET1 ,";
            ...
            strSQL += "from ";
            strSQL += "(select (a.RECORD_TIME+3/24) as time,a.WORK_SHIFT,a.WORK_TEAM,a.accum_charge,a.HOUR_CHARGE,a.GAS_USE,a.BLAST_AMOUNT,a.HOT_PRESS,COLD_PRESS,";
            ...
            strSQL += " (b.OXY_AMOUNT+b.N2_AMOUNT) AS RICH_OXY_AMOUNT,a.COKE_JET,a.COKE_JET1,a.COKE_RATIO,a.BLAST_SPEED, ";
            ...
            strSQL += " a.P_SOFT2_WATER,a.T_TEN_HEART,a.T_LD_HEART,a.T_LJ_HEART,b.OXY_AMOUNT AS R_LG_ACTIVE,b.N2_AMOUNT AS GY_OXY_AMOUNT,a.LT_RFH,a.SPEED_LMXJ ";
