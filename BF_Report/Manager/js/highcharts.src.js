 ...
 * @param {Number} doubles The amount of doubles
 * @param {String} decPoint The double point, defaults to the one given in the lang options
 * @param {String} thousandsSep The thousands separator, defaults to the one given in the lang options
 */
function numberFormat(number, doubles, decPoint, thousandsSep) {
	var lang = defaultOptions.lang,
		// http://kevin.vanzonneveld.net/techblog/article/javascript_equivalent_for_phps_number_format/
		n = number,
		c = doubles === -1 ?
			((n || 0).toString().split('.')[1] || '').length : // preserve doubles
			(isNaN(doubles = mathAbs(doubles)) ? 2 : doubles),
		d = decPoint === undefined ? lang.doublePoint : decPoint,
...
function formatSingle(format, val) {
	...
	if (floatRegex.test(format)) { // float
		doubles = format.match(decRegex);
		doubles = doubles ? doubles[1] : -1;
		val = numberFormat(
			val,
			doubles,
			lang.doublePoint,
...
function normalizeTickInterval(interval, multiples, magnitude, options) {
	...
	if (!multiples) {
		...
		// the allowdoubles option
		if (options && options.allowdoubles === false) {
			if (magnitude === 1) {
				multiples = [1, 2, 5, 10];
...
Point.prototype = {
	...
	tooltipFormatter: function (pointFormat) {
		...
		// Insert options for valuedoubles, valuePrefix, and valueSuffix
		var series = this.series,
			seriesTooltipOptions = series.tooltipOptions,
			valuedoubles = pick(seriesTooltipOptions.valuedoubles, ''),
		...
		each(series.pointArrayMap || ['y'], function (key) {
			...
			pointFormat = pointFormat.replace(key + '}', key + ':,.' + valuedoubles + 'f}');
		});
